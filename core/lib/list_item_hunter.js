  ...
  function processListItemPartials(pattern, patternlab) {
    ...
    if (matches !== null) {
      matches.forEach(function (liMatch) {
        ...
        var repeatedBlockTemplate = new Array(items.indexOf(loopNumberString)).fill(patternBlock);
        ...
        repeatedBlockTemplate.forEach((repeatedTemplate, index) => {
          var thisBlockTemplate = repeatedTemplate;
          var thisBlockHTML = "";
          ...
          allData = plutils.mergeData(allData, itemData !== undefined ? itemData[index] : {});
